...
def most_active(cur, niche, days=365, limit=10):
    cur.execute("""SELECT COALESCE(c.canonical_name, a.winner_name_raw) AS winner, c.country,
                          count(*) AS wins
                   FROM awards a LEFT JOIN companies c ON c.id=a.company_id
                   WHERE a.niche=%s AND a.award_date >= current_date - make_interval(days => %s)
                     AND a.winner_name_raw IS NOT NULL AND a.winner_name_raw <> ''
                   GROUP BY 1,2 ORDER BY wins DESC LIMIT %s""", (niche, days, limit))
    return cur.fetchall()

def hotspots(cur, niche, days=90, limit=8):
    cur.execute("""SELECT t.country, l.region_nuts,
                   ...
                   WHERE t.niche=%s AND t.notice_type='contract_notice'
                     ...
                     AND t.publication_date >= current_date - make_interval(days => %s)
                   GROUP BY 1,2 ORDER BY lots DESC, value_eur DESC LIMIT %s""", (niche, days, limit))
    return cur.fetchall()

def summary_paragraph(ops, spots, board, uncontested):
    parts = []
    if ops:
        total = sum((o["value_eur"] or 0) for o in ops)
        parts.append(f"<b>{len(ops)} new opportunities</b> this period, worth {eur(total)} combined.")
        biggest = max(ops, key=lambda o: (o["value_eur"] or 0))
        if biggest["value_eur"]:
            parts.append(f"Largest: “{esc(biggest.get('title_en') or biggest['lot_title'] or biggest['notice_title'])}” "
                         f"({esc(biggest['country'])}, {eur(biggest['value_eur'])}).")
    if spots:
...
def render(cur, niche, label, prefixes, green_re, days, title):
    ...
    spots = hotspots(cur, niche)
    active = most_active(cur, niche)
    ...
    for i, r in enumerate(board, 1):
        ...
        board_rows.append(
            ...
            f"<td>{esc(r['winner'])}<br><span class='muted' style='font-size:11px'>{sub}</span></td>"
            f"<td>{esc(r['country'])}</td><td>{r['wins']}</td><td>{eur(r['total'])}</td>"
            f"<td>{r['avg_bids'] if r['avg_bids'] is not None else '—'}</td>"
            f"<td>{esc(r['last_win'] or '—')}</td></tr>")
    ...
    spots_html = "".join(
        f"<tr><td>{esc(r['country'])}</td><td>{esc(r['region_nuts'])}</td>"
        f"<td>{r['lots']}</td><td>{eur(r['value_eur'])}</td></tr>"
...
<body><div class="wrap"><div class="header"><div class="brand">{esc(label)} Intelligence</div><h1>{esc(title)}</h1>
  ...
  <table><thead><tr><th>Country</th><th>Region (NUTS)</th><th>Opportunities</th><th>Value</th></tr></thead>
    <tbody>{spots_html}</tbody></table>
<h2>Lower-competition signals (12 mo)</h2>
  ...
  <table><thead><tr><th>#</th><th>Supplier</th><th>Country</th><th>Wins</th><th>Total awarded</th><th>Avg bids</th><th>Last win</th></tr></thead>
    <tbody>{board_html}</tbody></table>
  <h2>Most active suppliers (by number of wins, 12 mo)</h2>
  <table><thead><tr><th>#</th><th>Supplier</th><th>Country</th><th>Wins</th></tr></thead>
